    ...
    public class OrderService(IValidator<OrderAddRequest> Orderaddvalidator ,IValidator<OrderItemAddRequest> orderItemAddValidator, IValidator<OrderUpdateRequest> orderUpdateValidator, IValidator<OrderItemUpdateRequest> orderItemUpdateValidator, IOrderRepository _repo , ProductsMicroserviceClient productsMicroserviceClient, UsersMicroserviceClient usersMicroserviceClient, IRabbitMqPublisher rabbitMqPublisher) : IOrderService
    {
        public async Task<Result<OrderResponse>> AddOrder(OrderAddRequest request)
        {
            ...
            if (res.IsSuccess)
            {
                ...
                return Result.Ok(res.Value.ToDo() with { Email = user.Value.Email , UserPersonName = user.Value.PersonName});
            }
        ...
        public async Task<Result<OrderResponse>> GetOrderByCondition(FilterDefinition<Order> filter)
        {
            ...
            {
                orderResponse = orderResponse with { Email = user.Value.Email, UserPersonName = user.Value.PersonName };
            }
            ...
            foreach (var item in order.OrderItems)
            {
                ...
                {
                    itemResponse = itemResponse with { ProductName = product.Value.ProductName, Category = product.Value.Category };
                }
        ...
        public async Task<Result<IEnumerable<OrderResponse>>> GetOrders()
        {
            ...
            return Result.Ok(orderResponses.AsEnumerable());
        }

        public async Task<Result<IEnumerable<OrderResponse>>> GetOrdersByCondition(FilterDefinition<Order> filter)
        {
            var result = await _repo.GetOrdersByFilter(filter);
            if (!result.IsSuccess)
            {
                return Result.Fail(result.Errors);
            }
        ...
        public async Task<Result<OrderResponse>> UpdateOrder(OrderUpdateRequest request)
        {
            ...
            var orderResponse = updatedOrder.ToDo() with { Email = user.Value.Email, UserPersonName = user.Value.PersonName };
